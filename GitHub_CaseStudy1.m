...
lam = logspace(-6,0,10);    % lambda_min = 10^-6, lambda_max = 10^0, 10 lambda samples
                            % The simulations in the paper were performed using 200 lambda samples.
...
for j=1:length(tspan)       % Defining the f(t) (noise-free frequency) used in Case Study 1: Generator Trip Disturbance
    if tspan(j)<5
        f(j,1) = 50;
        derivExact(j,1) = 0;% The vector containing the samples of exact RoCoF
...
for j=1:length(tspan)-n
    ...
    for k=1:length(lam)
        ...
        q(k) = -2 * lambda * (y' * D * P * D' * D * P * A' * (z_hat-z_hat(1))) / (y'*y);
